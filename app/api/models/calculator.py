...
class CalculationRequest(BaseModel):
    ...
    b: Optional[float] = Field(None, description="Second operand (not required for sqrt)")
    
    @validator('a', 'b')
    def validate_numbers(cls, v):
        """Validate that numbers are finite"""
        if v is not None and not (isinstance(v, (int, float)) and v == v):  # Check for NaN
...
class OperationsResponse(BaseModel):
    ...
    operations: list[OperationInfo] = Field(..., description="List of available operations")
...
class HealthResponse(BaseModel):
    ...
    test_calculation: Optional[str] = Field(None, description="Test calculation performed")

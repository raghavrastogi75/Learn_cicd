...
class CalculatorService(LoggerMixin):
    ...
    async def calculate(self, operation: str, a: float, b: Optional[float] = None, session: AsyncSession = None) -> float:
    ...
    async def _store_calculation(self, operation: str, a: float, b: Optional[float], result: float, session: AsyncSession = None):
        """Store calculation in database"""
        try:
            ...
            if session:
                ...
                self.logger.warning("No database session provided for storing calculation")
    ...
    async def get_history(self, limit: int = 10, offset: int = 0, session: AsyncSession = None) -> List[Dict[str, Any]]:
        """Get calculation history"""
        try:
            if not session:
                raise ValueError("Database session is required")
            
            query = select(Calculation).order_by(Calculation.created_at.desc()).limit(limit).offset(offset)
    ...
    async def get_statistics(self, session: AsyncSession = None) -> Dict[str, Any]:
        ...
        try:
            ...
            ).group_by(Calculation.operation).order_by(func.count(Calculation.operation).desc()).limit(1)
            ...
            return {
                ...
                "most_used_operation": most_used_operation[0] if most_used_operation else None,

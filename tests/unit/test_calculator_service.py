...
class TestCalculatorService:
    ...
    async def test_get_history(self, calculator_service, mock_session):
        ...
        mock_session.execute.return_value.scalars.return_value.all.return_value = [mock_calculation]
        
        result = await calculator_service.get_history(limit=10, offset=0, session=mock_session)
    ...
    async def test_get_statistics(self, calculator_service, mock_session):
        ...
        mock_session.execute.return_value.scalar.side_effect = [100, 25.5, 10, 50]
        mock_session.execute.return_value.first.return_value = ("add", 30)

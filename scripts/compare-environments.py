...
class EnvironmentComparator:
    ...
    def get_environment_config(self, env: str) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run([
                "kubectl", "get", "configmap", env_info["config_map"],
                "-n", env_info["namespace"], "-o", "json"
    ...
    def display_comparison(self, comparison: Dict[str, Any]):
        ...
        # Headers
        headers = ["Environment", "Namespace", "Status", "Debug", "Log Level", "Rate Limit", "Features"]
        print(f"{headers[0]:<15} {headers[1]:<20} {headers[2]:<12} {headers[3]:<8} {headers[4]:<10} {headers[5]:<12} {headers[6]}")
        print("-" * 80)
        ...
        for env_name, env_data in comparison.items():
            ...
            print(f"{env_name:<15} {namespace:<20} {status:<12} {debug:<8} {log_level:<10} {rate_limit:<12} {features}")
